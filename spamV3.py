...
autoketik(f"""
{biru}----------------------------------------------------
{hijau}           Discord {putih}: {hijau}dhritzz
{hijau}           Ip Kamu {putih}  :{merah} {ip}
{hijau}           github {putih}: {hijau}dhritzz
             {G} {localtime}
{biru}----------------------------------------------------
""")

nomor = input(f"{putih}[{hijau}WA{putih}] {hijau}NOMOR TARGET +62 BG {putih}: ")
jum = int(input(f"{putih}[{hijau}WA{putih}] {hijau}JUMLAH NYA BERAPA BG TESERAH AG MAU ISI PERAPA SAYA TIDAK PEDULI {putih}: "))
for i in range(jum):
    ...
    print (f"{W}[{G}OK{W}] {G}SUCCESS SPAM WA NYA BANG")
